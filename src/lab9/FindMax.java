...
public class FindMax {
	...
	public void printMax() {
		// this is a single threaded version
		Thread t1 = new Thread( ()->{ findMax(0, 29);});
		Thread t2 = new Thread( ()->{ findMax(30, 59);});
		Thread t3 = new Thread( ()->{ findMax(60, 89);});
		t1.start(); t2.start();t3.start();
		int max = 0;
		while(t1.isAlive() || t2.isAlive() || t3.isAlive()) {}
		if(max_1[0] > max_1[1] && max_1[0] > max_1[2]) {
			max = max_1[0];
		}
		else if (max_1[1] > max_1[0] && max_1[1] > max_1[2]) {
			max = max_1[1];
		}
		else if(max_1[2] > max_1[1] && max_1[2] > max_1[1]) {
			max = max_1[2];
		}
